...
function sauvegarderEnseignant(matricule){
	...
	var poste=$("#Poste").val();
		...
		if(prenom=="" || prenom==null || !(/^[A-Za-z\s]+$/.test(prenom)) || prenom.length>50 ){
			...
			if(nom=="" || nom==null || !(/^[A-Za-z\s]+$/.test(nom)) || nom.length>50){
				...
				if(typeEmployer=="" || typeEmployer==null || !(/^[A-Za-z\s]+$/.test(typeEmployer))|| typeEmployer.length>200){
